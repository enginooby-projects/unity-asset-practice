...
/// </summary>
public class GOInteractController : MonoBehaviourSingleton<GOInteractController> {
  [PropertyOrder(-1)]
  [SerializeField, ShowIf(nameof(_isGeneral)), OnValueChanged(nameof(GetInteractorByType))]
  private ReferenceConcreteType<GOInteractor> _interactorType;
  [PropertyOrder(-1)]
  [SerializeField, InlineEditor, ShowIf(nameof(_isGeneral))]
  protected GOInteractor _interactor;
  [SerializeField, InlineEditor]
  private Selector _selector;
  [SerializeField]
  private GOInteractControlKeyPreset _keyPreset;
  ...
  private void GetInteractorByType() {
    if (_interactorType != null)
      _interactor = (GOInteractor)FindObjectOfType(_interactorType.Value);
  }

  protected virtual void GetInteractor() {
    _interactor = _interactor ?? FindObjectOfType<GOInteractor>();
    if (_interactorType != null)
      _interactor = (GOInteractor)FindObjectOfType(_interactorType.Value) ?? _interactorType.CreateInstance();
  }
  ...
  private void ProcessSwitchingEffect() {
    if (_keyPreset.IncrementEffectKey.IsTriggering) {
      _interactor.IncrementInteractingEffect();
    }

    if (_keyPreset.DecrementEffectKey.IsTriggering) {
      _interactor.DecrementInteractingEffect();
    }
  }
...
/// </summary>
public abstract class GOInteractController<TSelf, TInteractor> : GOInteractController
where TSelf : GOInteractController<TSelf, TInteractor>
where TInteractor : GOInteractor<TInteractor> {
  [PropertyOrder(-1)]
  ...
  protected override void GetInteractor() {
    ...
    _tInteractor = _tInteractor ?? FindObjectOfType<TInteractor>() ?? GOInteractor<TInteractor>.Instance;
