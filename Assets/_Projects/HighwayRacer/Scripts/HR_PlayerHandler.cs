...
namespace Project.HighwayRacer {
  ...
  public class HR_PlayerHandler : MonoBehaviour {
    ...
    private bool gameStarted {
      get { return HR_GamePlayHandler.Instance.gameStarted; }
    }
    ...
    void OnEnable() {
      ...
      hornSource = HR_CreateAudioSource.NewAudioSource(gameObject, "Horn", 10f, 100f, 1f,
        HR_HighwayRacerProperties.Instance.hornClip, true, false, false);
    }
    ...
    void OnCollisionEnter(Collision col) {
      ...
      colRelVel *= 1f - (Mathf.Abs(Vector3.Dot(transform.up, col.contacts[col.contacts.Length - 1].normal)) +
                         Mathf.Abs(Vector3.Dot(transform.up, col.contacts[0].normal)));
      ...
      if (colRelVel.magnitude * cos < HR_HighwayRacerProperties.Instance._minimumCollisionForGameOver &&
          colRelVel.magnitude * cos2 < HR_HighwayRacerProperties.Instance._minimumCollisionForGameOver)
        return;
