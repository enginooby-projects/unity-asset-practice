...
namespace Project.HighwayRacer {
  public class HR_GameOverPanel : MonoBehaviour {
    ...
    public int totalDistanceMoneyMP => HR_HighwayRacerProperties.Instance._totalDistanceMoneyMP;
    public int totalNearMissMoneyMP => HR_HighwayRacerProperties.Instance._totalNearMissMoneyMP;
    public int totalOverspeedMoneyMP => HR_HighwayRacerProperties.Instance._totalOverspeedMoneyMP;
    public int totalOppositeDirectionMP => HR_HighwayRacerProperties.Instance._totalOppositeDirectionMP;

    private void OnEnable() {
      HR_PlayerHandler.OnPlayerDied += HR_PlayerHandler_OnPlayerDied;
    }

    private void HR_PlayerHandler_OnPlayerDied(HR_PlayerHandler player) {
      StartCoroutine(DisplayResults(player));
    }

    private IEnumerator DisplayResults(HR_PlayerHandler player) {
      ...
      PlayerPrefs.SetInt("Currency",
        PlayerPrefs.GetInt("Currency", 0) + Mathf.FloorToInt(Mathf.Floor(player.distance * totalDistanceMoneyMP) +
                                                             (player.nearMisses * totalNearMissMoneyMP) +
                                                             Mathf.Floor(player.highSpeedTotal *
