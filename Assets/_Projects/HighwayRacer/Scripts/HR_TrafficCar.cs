...
namespace Project.HighwayRacer {
  ...
  public class HR_TrafficCar : MonoBehaviour {
    ...
    [Space(10)] public AudioClip engineSound;
    ...
    void Awake() {
      ...
      triggerCollider.size = new Vector3(bodyCollider.size.x * 1.5f, bodyCollider.size.y,
        bodyCollider.size.z + (bodyCollider.size.z * 3f));
      triggerCollider.center = new Vector3(bodyCollider.center.x, 0f,
        bodyCollider.center.z + (triggerCollider.size.z / 2f) - (bodyCollider.size.z / 2f));

      if (HR_GamePlayHandler.dayOrNight == HR_GamePlayHandler.DayOrNight.Night ||
          HR_GamePlayHandler.dayOrNight == HR_GamePlayHandler.DayOrNight.Rainy)
        headlightsOn = true;
      else
        headlightsOn = false;

      engineSoundSource =
        HR_CreateAudioSource.NewAudioSource(gameObject, "Engine Sound", 2f, 5f, 1f, engineSound, true, true, false);
    ...
    void FixedUpdate() {
      if (!immobilized) {
        desiredSpeed = Mathf.Clamp(maximumSpeed - Mathf.Lerp(maximumSpeed, 0f, (distance - 10f) / 50f), 0f,
          maximumSpeed);
      }
      else {
