...
//		Color selectedColor = new Color();
      Color selectedColor = pickedColor;
...
//		}
    ...
    void Update() {
      string currentColorString = pickedColor.ToString();

      if (colorPrice <= 0 && !unlocked) {
        PlayerPrefs.SetInt(HR_MainMenuHandler.Instance.currentCar.transform.name + "OwnedColor" + currentColorString,
          1);
        unlocked = true;
      }
    ...
    void BuyColor() {
      ...
      if (playerCoins >= colorPrice) {
        ...
        handler.BuyProperty(colorPrice,
          HR_MainMenuHandler.Instance.currentCar.transform.name + "OwnedColor" + currentColorString);
      }
    }
  }
}