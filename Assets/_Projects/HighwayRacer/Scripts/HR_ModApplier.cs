...
namespace Project.HighwayRacer {
  ...
  public class HR_ModApplier : MonoBehaviour {
    ...
    [Range(160f, 350f)] public float maxUpgradeSpeed = 220f;
    [Range(1f, 10f)] public float maxUpgradeHandling = 4;
    [Range(1f, 10f)] public float maxUpgradeBrake = 4;
    ...
    void Awake() {
      ...
      defHandling = highwaySteeringHelper;
      defMaxBrake = highwayBrakingHelper;
    ...
    private void FixedUpdate() {
      ...
      if (carController.speed > 20f)
        rigid.AddForce(-Vector3.forward * (carController.brakeInput * highwayBrakingHelper / 2.5f),
    ...
    void CreateSiren() {
      ...
      attachedFrontSiren.transform.SetParent(GetComponent<RCC_CarControllerV3>().transform, false);
      attachedRearSiren.transform.SetParent(GetComponent<RCC_CarControllerV3>().transform, false);
      ...
      GameObject rearSiren = (GameObject) Instantiate(HR_HighwayRacerProperties.Instance.attachableSiren,
        GetComponent<RCC_CarControllerV3>().transform.position, transform.rotation * Quaternion.Euler(0f, 180f, 0f));
      ...
      for (int i = 0; i < 2; i++) {
        ...
        if (frontSirenLights[i].transform.localPosition.x >= 0f) {
          ...
          frontSirenLights[i].transform.localPosition = new Vector3(.25f, frontSirenLights[i].transform.localPosition.y,
            frontSirenLights[i].transform.localPosition.z);
        }
        else {
          frontSirenLights[i].transform.position = headLights[i].transform.position;
          frontSirenLights[i].transform.localPosition = new Vector3(-.25f,
            frontSirenLights[i].transform.localPosition.y,
            frontSirenLights[i].transform.localPosition.z);
        }
      }
      ...
      for (int i = 0; i < 2; i++) {
        ...
        if (rearSirenLights[i].transform.localPosition.x >= 0f) {
          ...
          rearSirenLights[i].transform.localPosition = new Vector3(.25f, rearSirenLights[i].transform.localPosition.y,
            rearSirenLights[i].transform.localPosition.z);
        }
        else {
          rearSirenLights[i].transform.position = brakeLights[i].transform.position;
          rearSirenLights[i].transform.localPosition = new Vector3(-.25f, rearSirenLights[i].transform.localPosition.y,
    ...
    public void ChangeWheels(RCC_CarControllerV3 car, GameObject wheel) {
      RCC_Customization.ChangeWheels(car, selectedWheel, true);
    }
    ...
    void CheckGroundGap() {
      ...
      if (Physics.Raycast(wheel.transform.position, -Vector3.up, out hit, 10f))
        transform.position = new Vector3(transform.position.x,
          hit.point.y + distancePivotBetweenWheel + (wheel.radius) + (wheel.suspensionDistance / 2f),
