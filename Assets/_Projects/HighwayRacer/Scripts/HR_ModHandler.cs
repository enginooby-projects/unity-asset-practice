...
namespace Project.HighwayRacer {
  public class HR_ModHandler : MonoBehaviour {
    ...
    //UI Panels
    [Header("Modify Panels")] public GameObject colorClass;
    public GameObject wheelClass;
    public GameObject upgradesClass;

    //UI Buttons
    [Header("Modify Buttons")] public Button bodyPaintButton;
    public Button rimButton;
    public Button upgradeButton;
    private Color orgButtonColor;

    //UI Sliders
    [Header("Power Bars")] public Slider speedBar;
    public Slider handlingBar;
    public Slider brakeBar;

    [Header("Maximum Power Bars")] public Slider maxSpeedBar;
    public Slider maxHandlingBar;
    public Slider maxBrakeBar;

    //UI Texts
    [Header("Upgrade Levels Texts")] public Text speedUpgradeLevel;
    ...
    void Update() {
      ...
      if (speedBar)
        speedBar.value = Mathf.Lerp(speedBar.value, currentApplier.maxSpeed / 350f, Time.deltaTime * 5f);
      if (handlingBar)
        handlingBar.value =
          Mathf.Lerp(handlingBar.value, currentApplier.highwaySteeringHelper / 10f, Time.deltaTime * 5f);
      if (brakeBar)
        brakeBar.value = Mathf.Lerp(brakeBar.value, currentApplier.highwayBrakingHelper / 10f, Time.deltaTime * 5f);

      if (maxSpeedBar)
        maxSpeedBar.value = Mathf.Lerp(maxSpeedBar.value, currentApplier.maxUpgradeSpeed / 350f, Time.deltaTime * 5f);
      if (maxHandlingBar)
        maxHandlingBar.value =
          Mathf.Lerp(maxHandlingBar.value, currentApplier.maxUpgradeHandling / 10f, Time.deltaTime * 5f);
      if (maxBrakeBar)
      ...
      if (sirenUpgradeLevel)
        sirenUpgradeLevel.text =
          currentApplier.isSirenPurchased && currentApplier.attachedFrontSiren.activeSelf ? "ON" : "OFF";
      if (nosUpgradeLevel)
