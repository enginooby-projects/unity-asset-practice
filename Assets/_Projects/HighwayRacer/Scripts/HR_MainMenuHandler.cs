...
namespace Project.HighwayRacer {
  public class HR_MainMenuHandler : MonoBehaviour {
    ...
    public GameObject optionsMenu_Mobile;
    ...
    [Header("UI Loading Section")] public GameObject loadingScreen;
    ...
    void Awake() {
      ...
      if (HR_HighwayRacerProperties.Instance.mainMenuClips != null &&
        ...
        mainMenuSoundtrack = HR_CreateAudioSource.NewAudioSource(gameObject, "Main Menu Soundtrack", 0f, 0f, 1f,
          HR_HighwayRacerProperties.Instance.mainMenuClips[
            UnityEngine.Random.Range(0, HR_HighwayRacerProperties.Instance.mainMenuClips.Length)], true, true, false);
    ...
    void CreateCars() {
      ...
      for (int i = 0; i < createdCars.Length; i++) {
        createdCars[i] = (RCC.SpawnRCC(HR_PlayerCars.Instance.cars[i].playerCar.GetComponent<RCC_CarControllerV3>(),
          carSpawnLocation.position, carSpawnLocation.rotation, false, false, false)).gameObject;
//			createdCars [i].GetComponent<RCC_CarControllerV3> ().lowBeamHeadLightsOn = true;
    ...
    public void EnableMenu(GameObject activeMenu) {
      if (activeMenu == optionsMenu || activeMenu == optionsMenu_Mobile) {
        if (RCC_Settings.Instance.controllerType == RCC_Settings.ControllerType.Mobile)
          activeMenu = optionsMenu_Mobile;
        else
          activeMenu = optionsMenu;
      }

      optionsMenu.SetActive(false);
      optionsMenu_Mobile.SetActive(false);
