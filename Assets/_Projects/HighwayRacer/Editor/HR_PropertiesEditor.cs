...
public class HR_PropertiesEditor : Editor {
  ...
  public override void OnInspectorGUI() {
    ...
    EditorGUILayout.LabelField(
      "This editor will keep update necessary .asset files in your project. Don't change directory of the ''Resources/HR_Assets''.",
    ...
    prop._minimumSpeedForGainScore =
      EditorGUILayout.IntField("Min Speed For Gain Score", prop._minimumSpeedForGainScore);
    prop._minimumSpeedForHighSpeed =
      EditorGUILayout.IntField("Min Speed For High Speed", prop._minimumSpeedForHighSpeed);
    prop._minimumCollisionForGameOver =
      EditorGUILayout.IntField("Min Col For Game Over", prop._minimumCollisionForGameOver);

    EditorGUILayout.Space();

    GUILayout.Label("Default Config At First Initialization", EditorStyles.boldLabel);
    EditorGUILayout.HelpBox(
      "Default settings when there are no any saved PlayerPrefs data. At first init only on target platform.",
    ...
    prop._totalDistanceMoneyMP =
      EditorGUILayout.IntField("Total Distance MP", prop._totalDistanceMoneyMP, GUILayout.ExpandWidth(true));
    prop._totalNearMissMoneyMP =
      EditorGUILayout.IntField("Total Near Miss MP", prop._totalNearMissMoneyMP, GUILayout.ExpandWidth(true));
    prop._totalOverspeedMoneyMP =
      EditorGUILayout.IntField("Total Over Speed MP", prop._totalOverspeedMoneyMP, GUILayout.ExpandWidth(true));
    prop._totalOppositeDirectionMP = EditorGUILayout.IntField("Total Opposite Direction MP",
      prop._totalOppositeDirectionMP, GUILayout.ExpandWidth(true));

    EditorGUILayout.Space();

    GUILayout.Label("Sound Effects", EditorStyles.boldLabel);

    EditorGUILayout.PropertyField(serializedObject.FindProperty("mainMenuClips"),
      new GUIContent("Main Menu Soundtracks"), true);
    EditorGUILayout.PropertyField(serializedObject.FindProperty("gameplayClips"),
      new GUIContent("GamePlay Soundtracks"), true);
    EditorGUILayout.Space();
    EditorGUILayout.PropertyField(serializedObject.FindProperty("buttonClickAudioClip"),
      new GUIContent("Button Click SFX"));
    EditorGUILayout.PropertyField(serializedObject.FindProperty("nearMissAudioClip"), new GUIContent("Near Miss SFX"));
    EditorGUILayout.PropertyField(serializedObject.FindProperty("labelSlideAudioClip"),
      new GUIContent("Label Slide SFX"));
    EditorGUILayout.PropertyField(serializedObject.FindProperty("countingPointsAudioClip"),
      new GUIContent("Counting Points SFX"));
    EditorGUILayout.Space();
    EditorGUILayout.PropertyField(serializedObject.FindProperty("bombTimerAudioClip"),
      new GUIContent("Bomb Timer Beep SFX"));
    EditorGUILayout.Space();
    ...
    prop._1MMoneyForTesting =
      EditorGUILayout.ToggleLeft("1M Money For Testing", prop._1MMoneyForTesting, GUILayout.ExpandWidth(true));
    ...
    EditorGUILayout.PropertyField(serializedObject.FindProperty("explosionEffect"),
      new GUIContent("Explosion Particle Effects"));

    EditorGUILayout.Space();

    EditorGUILayout.PropertyField(serializedObject.FindProperty("trafficCarsLayer"),
      new GUIContent("Layer Of The Traffic Cars"));
    ...
    if (GUILayout.Button("Reset To Defaults")) {
      if (EditorUtility.DisplayDialog("Delete All PlayerPrefs Data",
            "Are you sure to reset all settings to default? Don't worry, your cars, prices, upgrades will stay the same.",
            "Yes", "Nope")) {
        ResetToDefaults();
    ...
    if (GUILayout.Button("Delete All PlayerPrefs")) {
      if (EditorUtility.DisplayDialog("Delete All PlayerPrefs Data",
            "Are you sure to delete all local playerprefs data?", "Yes", "Nope")) {
