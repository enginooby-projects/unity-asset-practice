...
public class RCC_LightEditor : Editor {
  ...
  public override void OnInspectorGUI() {
    ...
    EditorGUILayout.LabelField(
      "RCC lights will receive inputs from parent car controller and adjusts intensity for lights. You can choose which type of light you want to use below. You won't need to specify left or right indicator lights.",
      EditorStyles.helpBox);
    EditorGUILayout.LabelField("''Important'' or ''Not Important'' modes (Pixel or Vertex) overrided by RCC_Settings.",
      EditorStyles.helpBox);
    EditorGUILayout.Space();
    ...
    if (!prop.GetComponent<LensFlare>()) {
      ...
      EditorGUILayout.Space();
      EditorGUILayout.LabelField(
        "RCC uses ''Interpolation'' mode for all rigidbodies. Therefore, lights at front of the vehicle will blink while on high speeds. To fix this, select your RCC layer in LensFlare component as ignored layer. RCC_Light script will simulate lens flares depending on camera distance and angle.''.",
        EditorStyles.helpBox);
      EditorGUILayout.Space();

      EditorGUILayout.PropertyField(serializedObject.FindProperty("flareBrightness"),
        new GUIContent("Lens Flare Brightness"), false);
    }

    EditorGUILayout.PropertyField(serializedObject.FindProperty("inertia"), new GUIContent("Lens Flare Fade Speed"),
      false);
    ...
    if (!prop.GetComponentInChildren<TrailRenderer>()) {
      if (GUILayout.Button("Create Trail")) {
        ...
        for (int i = 0; i < lights.Length; i++) {
          ...
          GameObject newTrail = GameObject.Instantiate(RCC_Settings.Instance.lightTrailers,
            lights[i].transform.position, lights[i].transform.rotation, lights[i].transform);
  ...
  void CheckLights() {
    ...
    Vector3 relativePos = prop.GetComponentInParent<RCC_CarControllerV3>().transform
      .InverseTransformPoint(prop.transform.position);
