...
public class RCC_SceneGUI : EditorWindow {
  ...
  private static void OnScene(SceneView sceneview) {
    ...
    if (GUILayout.Button(new GUIContent(" X ",
          "Close the in-scene window. You can re-enable in Tools --> BCG --> RCC")))
      Disable();
    ...
    if (Selection.activeGameObject != null) {
      if (Selection.activeGameObject.GetComponentInParent<RCC_CarControllerV3>()) {
        ...
        if (GUILayout.Button(new GUIContent(mirrorIcon, "Add/Select mirrors attached to selected vehicle")))
          RCC_EditorWindows.CreateMirrors(Selection.activeGameObject.GetComponentInParent<RCC_CarControllerV3>()
            .gameObject);
        ...
        if (Selection.activeGameObject.GetComponent<RCC_Light>()) {
          if (GUILayout.Button(new GUIContent(indicatorlightIcon, "Duplicate light to opposite direction"))) {
            ...
            duplicatedLight.transform.localPosition = new Vector3(-Selection.activeGameObject.transform.localPosition.x,
              Selection.activeGameObject.transform.localPosition.y,
