...
public class RCC_AICarController : MonoBehaviour {
  ...
  void Update() {
    ...
    navigator.transform.localPosition +=
      Vector3.forward * carController.FrontLeftWheelCollider.transform.localPosition.z;
  ...
  void Navigation() {
    ...
    float navigatorInput =
      Mathf.Clamp(transform.InverseTransformDirection(navigator.desiredVelocity).x * 1.5f, -1f, 1f);
      ...
      case NavigationMode.FollowWaypoints:
        ...
        if (!reversingNow) {
          throttleInput = (distanceToNextWaypoint < (nextWaypointPassDistance * (carController.speed / 30f)))
            ? (Mathf.Clamp01(currentWaypoint.targetSpeed - carController.speed))
            : 1f;
          ...
          brakeInput = (distanceToNextWaypoint < (nextWaypointPassDistance * (carController.speed / 30f)))
            ? (Mathf.Clamp01(carController.speed - currentWaypoint.targetSpeed))
            : 0f;
          handbrakeInput = 0f;
      ...
      case NavigationMode.FollowTarget:
        ...
        if (!reversingNow) {
          throttleInput = distanceToTarget < (stopFollowDistance * Mathf.Lerp(1f, 5f, carController.speed / 50f))
            ? Mathf.Lerp(-5f, 1f, distanceToTarget / (stopFollowDistance / 1f))
            : 1f;
          throttleInput *= Mathf.Clamp01(Mathf.Lerp(10f, 0f, (carController.speed) / maximumSpeed));
          brakeInput = distanceToTarget < (stopFollowDistance * Mathf.Lerp(1f, 5f, carController.speed / 50f))
            ? Mathf.Lerp(5f, 0f, distanceToTarget / (stopFollowDistance / 1f))
            : 0f;
          handbrakeInput = 0f;
  ...
  void FixedRaycasts() {
    ...
    for (int i = 0; i < anglesOfRaycasts.Length; i++) {
      Debug.DrawRay(pivotPos,
        Quaternion.AngleAxis(anglesOfRaycasts[i], transform.up) * transform.forward * raycastLength, Color.white);

      if (Physics.Raycast(pivotPos, Quaternion.AngleAxis(anglesOfRaycasts[i], transform.up) * transform.forward,
            out hit, raycastLength, obstacleLayers) && !hit.collider.isTrigger && hit.transform.root != transform) {
        switch (navigationMode) {
          case NavigationMode.FollowWaypoints:

            Debug.DrawRay(pivotPos,
              Quaternion.AngleAxis(anglesOfRaycasts[i], transform.up) * transform.forward * raycastLength, Color.red);
            casted = true;
