...
public class RCC_Exhaust : RCC_Core {
  ...
  void Start() {
    ...
    if (flame) {
      ...
      flameSource = NewAudioSource(RCCSettings.audioMixer, gameObject, "Exhaust Flame AudioSource", 10f, 25f, .5f,
        RCCSettings.exhaustFlameClips[0], false, false, false);

      if (flameLight)
        flameLight.renderMode = RCCSettings.useLightsAsVertexLights
  ...
  void Flame() {
    if (carController.engineRunning) {
      ...
      if (((carController.useExhaustFlame && carController.engineRPM >= 5000 && carController.engineRPM <= 5500 &&
            carController.throttleInput <= .25f && flameTime <= .5f) || carController.boostInput >= .75f) ||
          previewFlames) {
  ...
  private void LensFlare() {
    ...
    float distanceTocam = Vector3.Distance(transform.position,
      RCC_SceneManager.Instance.activePlayerCamera.thisCam.transform.position);
    float angle = Vector3.Angle(transform.forward,
      RCC_SceneManager.Instance.activePlayerCamera.thisCam.transform.position - transform.position);
