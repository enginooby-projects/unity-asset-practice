...
public class RCC_WheelCollider : RCC_Core {
  ...
  private List<RCC_WheelCollider>
    allWheelColliders = new List<RCC_WheelCollider>(); // All wheelcolliders attached to this vehicle.
  ...
  [Space()] public bool canPower = false; //	Can this wheel power?
  ...
  [Space()] public float width = .275f; //	Width.
  ...
  private RCC_GroundMaterials physicsMaterials {
    get { return RCCGroundMaterials; }
  } // Getting instance of Configurable Ground Materials.

  private RCC_GroundMaterials.GroundMaterialFrictions[] physicsFrictions {
    get { return RCCGroundMaterials.frictions; }
  }
  ...
  //	Slips
  public float wheelSlipAmountForward = 0f; // Forward slip.
  public float wheelSlipAmountSideways = 0f; // Sideways slip.
  public float totalSlip = 0f; // Total amount of forward and sideways slips.
  ...
  void Start() {
    ...
    audioSource = NewAudioSource(RCCSettings.audioMixer, carController.gameObject, "Skid Sound AudioSource", 5f, 50f,
      0f, audioClip, true, true, false);
    ...
    if (!RCCSettings.dontUseAnyParticleEffects) {
      for (int i = 0; i < RCCGroundMaterials.frictions.Length; i++) {
        GameObject ps = (GameObject) Instantiate(RCCGroundMaterials.frictions[i].groundParticles, transform.position,
  ...
  private void CheckBehavior() {
    ...
    if (behavior != null) {
      forwardFrictionCurve = SetFrictionCurves(forwardFrictionCurve, behavior.forwardExtremumSlip,
        behavior.forwardExtremumValue, behavior.forwardAsymptoteSlip, behavior.forwardAsymptoteValue);
      sidewaysFrictionCurve = SetFrictionCurves(sidewaysFrictionCurve, behavior.sidewaysExtremumSlip,
        behavior.sidewaysExtremumValue, behavior.sidewaysAsymptoteSlip, behavior.sidewaysAsymptoteValue);
    }
  ...
  private void WheelAlign() {
    ...
    if (!wheelModel) {
      Debug.LogError(transform.name + " wheel of the " + carController.transform.name +
                     " is missing wheel model. This wheel is disabled");
      enabled = false;
      return;
    }
    ...
    if (carController.wheelDamage != RCC_CarControllerV3.WheelDamage.Off) {
  ...
  private void SkidMarks() {
    ...
    totalSlip = Mathf.Lerp(totalSlip, ((wheelSlipAmountSideways + wheelSlipAmountForward) / 2f),
    ...
    if (!RCCSettings.dontUseSkidmarks) {
      ...
      if (totalSlip > physicsFrictions[groundIndex].slip) {
        ...
        if (rigid.velocity.magnitude > 1f && isGrounded && wheelHit.normal != Vector3.zero &&
            wheelHit.point != Vector3.zero && skidPoint != Vector3.zero && Mathf.Abs(skidPoint.x) > 1f &&
            Mathf.Abs(skidPoint.z) > 1f)
          lastSkidmark = skidmarksManager.AddSkidMark(skidPoint, wheelHit.normal,
            totalSlip - physicsFrictions[groundIndex].slip, lastSkidmark, groundIndex, width);
        else
          lastSkidmark = -1;
      }
      else {
  ...
  private void Frictions() {
    ...
    sidewaysFrictionCurve.stiffness =
      (physicsFrictions[groundIndex].sidewaysStiffness * hbInput * tractionHelpedSidewaysStiffness);
  ...
  private void Drift() {
    ...
    // Forward
    if (wheelCollider == carController.FrontLeftWheelCollider.wheelCollider ||
        wheelCollider == carController.FrontRightWheelCollider.wheelCollider) {
  ...
  private void Audio() {
    ...
    if ((bumpForce) >= 5000f) {
      ...
      AudioSource bumpSound = NewAudioSource(RCCSettings.audioMixer, carController.gameObject, "Bump Sound AudioSource",
        5f, 50f, (bumpForce - 5000f) / 3000f, RCCSettingsInstance.bumpClip, false, true, true);
  ...
  public void ApplySteering(float steerInput, float angle) {
    ...
    if (steerInput > 0f) {
      if (transform.localPosition.x < 0)
        wheelCollider.steerAngle = (Mathf.Deg2Rad * angle * 2.55f) *
                                   (Mathf.Rad2Deg * Mathf.Atan(2.55f / (6 + (1.5f / 2))) * steerInput);
      else
        wheelCollider.steerAngle = (Mathf.Deg2Rad * angle * 2.55f) *
                                   (Mathf.Rad2Deg * Mathf.Atan(2.55f / (6 - (1.5f / 2))) * steerInput);
    }
    ...
    if (carController.wheelDamage == RCC_CarControllerV3.WheelDamage.Mechanical) {
      if (transform.localPosition.x < 0)
  ...
  private bool OverTorque() {
    if (carController.speed > carController.maxspeed ||
        (carController.speed > carController.gears[carController.currentGear].maxSpeed &&
         carController.engineRPM > (carController.maxEngineRPM)) || !carController.engineRunning)
      return true;
  ...
  public WheelFrictionCurve SetFrictionCurves(WheelFrictionCurve curve, float extremumSlip, float extremumValue,
    float asymptoteSlip, float asymptoteValue) {
    ...
    if (Application.isPlaying) {
      ...
      float extension =
        (-wheelCollider.transform.InverseTransformPoint(hit.point).y -
         (wheelCollider.radius * transform.lossyScale.y)) / wheelCollider.suspensionDistance;
      Debug.DrawLine(hit.point, hit.point + transform.up * (hit.force / rigid.mass),
