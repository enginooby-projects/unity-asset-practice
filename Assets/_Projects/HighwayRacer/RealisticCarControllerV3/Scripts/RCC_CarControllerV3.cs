...
public class RCC_CarControllerV3 : RCC_Core {
  ...
  public bool
    canControl =
      true; // Enables / Disables controlling the vehicle. If enabled, can receive all inputs from InputManager.
  ...
  public float brakeTorque = 3500f; // Maximum Brake Torque.,
  ...
  public float
    antiRollVertical =
      0f; // Anti Roll Vertical Force For Preventing Flip Overs And Stability. I know it doesn't exist, but it can improve gameplay if you have high COM vehicles like monster trucks.
  ...
  public bool
    useRevLimiter = true; // Rev limiter above maximum engine RPM. Cuts gas when RPM exceeds maximum engine RPM.

  public bool useExhaustFlame = true; // Exhaust blows flame when driver cuts gas at certain RPMs.

  public bool runEngineAtAwake {
    get { return RCCSettings.runEngineAtAwake; }
  } // Engine running at Awake?
  ...
  [Range(1f, 50f)] public float steeringSmootherFactor = 15f; // Steering smoother multiplier.
  ...
  [Range(.1f, .9f)]
  public float clutchInertia = .25f; //	Adjusting clutch faster at lower values. Higher values for smooth clutch.

  public float gearShiftUpRPM = 5500f; //	Shifting up when engine RPM is high enough.
  ...
  internal bool
    canGoReverseNow =
      false; //	If speed is low enough and player pushes the brake button, enable this bool to go reverse.

  public float launched = 0f;

  public bool autoReverse {
    get {
      if (!externalController) return RCCSettings.autoReverse;
      else return true;
    }
  ...
  public bool automaticGear {
    get {
      if (!externalController) return RCCSettings.useAutomaticGear;
      else return true;
    }
  ...
  public bool useAutomaticClutch {
    get { return RCCSettingsInstance.useAutomaticClutch; }
  }
  ...
  private float
    minimumVertDistanceForDamagedMesh =
      .002f; // Comparing Original Vertex Positions Between Last Vertex Positions To Decide Mesh Is Repaired Or Not.
  ...
  public GameObject contactSparkle {
    get { return RCCSettings.contactParticles; }
  ...
  void Awake() {
    ...
    GetComponentInChildren<WheelCollider>().ConfigureVehicleSubsteps(configureVehicleSubsteps.speedThreshold,
      configureVehicleSubsteps.stepsBelowThreshold, configureVehicleSubsteps.stepsAboveThreshold);
  ...
  private void CreateAudios() {
    switch (audioType) {
      case AudioType.OneSource:
        ...
        engineSoundHigh = NewAudioSource(RCCSettings.audioMixer, gameObject, engineSoundPosition,
          "Engine Sound High AudioSource", 5, 50, 0, engineClipHigh, true, true, false);

        if (autoCreateEngineOffSounds) {
          engineSoundHighOff = NewAudioSource(RCCSettings.audioMixer, gameObject, engineSoundPosition,
            "Engine Sound High Off AudioSource", 5, 50, 0, engineClipHigh, true, true, false);

          NewLowPassFilter(engineSoundHighOff, 3000f);
        }
        else {
          engineSoundHighOff = NewAudioSource(RCCSettings.audioMixer, gameObject, engineSoundPosition,
            "Engine Sound High Off AudioSource", 5, 50, 0, engineClipHighOff, true, true, false);
        }
      ...
      case AudioType.TwoSource:
        ...
        engineSoundHigh = NewAudioSource(RCCSettings.audioMixer, gameObject, engineSoundPosition,
          "Engine Sound High AudioSource", 5, 50, 0, engineClipHigh, true, true, false);
        engineSoundLow = NewAudioSource(RCCSettings.audioMixer, gameObject, engineSoundPosition,
          "Engine Sound Low AudioSource", 5, 25, 0, engineClipLow, true, true, false);

        if (autoCreateEngineOffSounds) {
          engineSoundHighOff = NewAudioSource(RCCSettings.audioMixer, gameObject, engineSoundPosition,
            "Engine Sound High Off AudioSource", 5, 50, 0, engineClipHigh, true, true, false);
          engineSoundLowOff = NewAudioSource(RCCSettings.audioMixer, gameObject, engineSoundPosition,
            "Engine Sound Low Off AudioSource", 5, 25, 0, engineClipLow, true, true, false);
          ...
          engineSoundHighOff = NewAudioSource(RCCSettings.audioMixer, gameObject, engineSoundPosition,
            "Engine Sound High Off AudioSource", 5, 50, 0, engineClipHighOff, true, true, false);
          engineSoundLowOff = NewAudioSource(RCCSettings.audioMixer, gameObject, engineSoundPosition,
            "Engine Sound Low Off AudioSource", 5, 25, 0, engineClipLowOff, true, true, false);
        }
      ...
      case AudioType.ThreeSource:
        ...
        engineSoundHigh = NewAudioSource(RCCSettings.audioMixer, gameObject, engineSoundPosition,
          "Engine Sound High AudioSource", 5, 50, 0, engineClipHigh, true, true, false);
        engineSoundMed = NewAudioSource(RCCSettings.audioMixer, gameObject, engineSoundPosition,
          "Engine Sound Medium AudioSource", 5, 50, 0, engineClipMed, true, true, false);
        engineSoundLow = NewAudioSource(RCCSettings.audioMixer, gameObject, engineSoundPosition,
          "Engine Sound Low AudioSource", 5, 25, 0, engineClipLow, true, true, false);
        ...
        if (autoCreateEngineOffSounds) {
          engineSoundHighOff = NewAudioSource(RCCSettings.audioMixer, gameObject, engineSoundPosition,
            "Engine Sound High Off AudioSource", 5, 50, 0, engineClipHigh, true, true, false);
          engineSoundMedOff = NewAudioSource(RCCSettings.audioMixer, gameObject, engineSoundPosition,
            "Engine Sound Medium Off AudioSource", 5, 50, 0, engineClipMed, true, true, false);
          engineSoundLowOff = NewAudioSource(RCCSettings.audioMixer, gameObject, engineSoundPosition,
            "Engine Sound Low Off AudioSource", 5, 25, 0, engineClipLow, true, true, false);
          ...
          engineSoundHighOff = NewAudioSource(RCCSettings.audioMixer, gameObject, engineSoundPosition,
            "Engine Sound High Off AudioSource", 5, 50, 0, engineClipHighOff, true, true, false);
          engineSoundMedOff = NewAudioSource(RCCSettings.audioMixer, gameObject, engineSoundPosition,
            "Engine Sound Medium Off AudioSource", 5, 50, 0, engineClipMedOff, true, true, false);
          engineSoundLowOff = NewAudioSource(RCCSettings.audioMixer, gameObject, engineSoundPosition,
            "Engine Sound Low Off AudioSource", 5, 25, 0, engineClipLowOff, true, true, false);
        }
    ...
    engineSoundIdle = NewAudioSource(RCCSettings.audioMixer, gameObject, engineSoundPosition,
      "Engine Sound Idle AudioSource", 5, 25, 0, engineClipIdle, true, true, false);
    reversingSound = NewAudioSource(RCCSettings.audioMixer, gameObject, gearSoundPosition, "Reverse Sound AudioSource",
      10, 50, 0, reversingClip, true, false, false);
    windSound = NewAudioSource(RCCSettings.audioMixer, gameObject, windSoundPosition, "Wind Sound AudioSource", 1, 10,
      0, windClip, true, true, false);
    brakeSound = NewAudioSource(RCCSettings.audioMixer, gameObject, "Brake Sound AudioSource", 1, 10, 0, brakeClip,
      true, true, false);

    if (useNOS)
      NOSSound = NewAudioSource(RCCSettings.audioMixer, gameObject, exhaustSoundPosition, "NOS Sound AudioSource", 5,
        10, .5f, NOSClip, true, false, false);
    if (useNOS || useTurbo)
      blowSound = NewAudioSource(RCCSettings.audioMixer, gameObject, exhaustSoundPosition, "NOS Blow", 1f, 10f, .5f,
        null, false, false, false);
    if (useTurbo) {
      turboSound = NewAudioSource(RCCSettings.audioMixer, gameObject, turboSoundPosition, "Turbo Sound AudioSource",
        .1f, .5f, 0f, turboClip, true, true, false);
  ...
  public void InitGears() {
    ...
    for (int i = 0; i < gears.Length; i++) {
      ...
      targetSpeedForGear[i] = (int) (Mathf.Lerp(0, maxspeed * Mathf.Lerp(0f, 1f, gearShiftingThreshold),
        ((float) (i + 1) / (float) (gears.Length))));
    }
    ...
    if (deformableMeshFilters.Length == 0) {
      ...
      foreach (MeshFilter mf in allMeshFilters) {
        if (!mf.mesh.isReadable)
          Debug.LogError("Not deformable mesh detected. Mesh of the " + mf.transform.name +
                         " isReadable is false; Read/Write must be enabled in import settings for this model!");
        else if (!mf.transform.IsChildOf(FrontLeftWheelTransform) &&
                 !mf.transform.IsChildOf(FrontRightWheelTransform) && !mf.transform.IsChildOf(RearLeftWheelTransform) &&
  ...
  public IEnumerator StartEngineDelayed() {
    if (!engineRunning) {
      engineStartSound = NewAudioSource(RCCSettings.audioMixer, gameObject, engineSoundPosition,
        "Engine Start AudioSource", 1, 10, 1, engineStartClip, false, true, true);
  ...
  public void Repair() {
    if (!repaired && repairNow) {
      ...
      for (int i = 0; i < allWheelColliders.Length; i++) {
        ...
        if (Mathf.Abs(allWheelColliders[i].damagedCamber) > .05f || Mathf.Abs(allWheelColliders[i].damagedToe) > .05f ||
            Mathf.Abs(allWheelColliders[i].damagedCaster) > .05f) {
          repaired = false;
        }
        else {
  ...
  private void DeformMesh(Mesh mesh, Vector3[] originalMesh, Collision collision, float cos, Transform meshTransform,
    Quaternion rot) {
      ...
      for (int i = 0; i < vertices.Length; i++) {
        if ((point - vertices[i]).magnitude < damageRadius) {
          vertices[i] += rot * ((localVector * (damageRadius - (point - vertices[i]).magnitude) / damageRadius) * cos +
                                (new Vector3(Mathf.Sin(vertices[i].y * 1000), Mathf.Sin(vertices[i].z * 1000),
    ...
    if (SteeringWheel) {
      ...
      switch (steeringWheelRotateAround) {
        case SteeringWheelRotateAround.XAxis:
          SteeringWheel.transform.localRotation = orgSteeringWheelRot *
                                                  Quaternion.AngleAxis(
                                                    ((steerInput * steerAngle) * -steeringWheelAngleMultiplier),
                                                    Vector3.right);
          break;
  ...
  void Update() {
    ...
    //Reversing Bool.
    if (brakeInput > .9f && transform.InverseTransformDirection(rigid.velocity).z < 1f && canGoReverseNow &&
        automaticGear && !semiAutomaticGear && !changingGear && direction != -1)
      StartCoroutine(ChangeGear(-1));
    else if (throttleInput < .1f && transform.InverseTransformDirection(rigid.velocity).z > -1f && direction == -1 &&
             !changingGear && automaticGear && !semiAutomaticGear)
  ...
  private void Inputs() {
    if (canControl) {
      if (!externalController) {
        ...
        float angle = HR_CalculateAngle.CalculateAngle(transform.rotation,
          Quaternion.identity * Quaternion.AngleAxis(steerInput * steerAngle, Vector3.up));
  ...
  private void LimitSteering() {
    ...
    maxSteerInput =
      Mathf.Clamp((1f - (Mathf.Abs(sidewaysSlip) * counterSteeringFactor)), -1f,
  ...
  private void GetExternalInputs() {
    switch (RCCSettings.selectedControllerType) {
      ...
      case RCC_Settings.ControllerType.XBox360One:
        ...
        if (!string.IsNullOrEmpty(RCCSettingsInstance.Xbox_startEngineKB) &&
            RCC_InputManager.GetButtonDown(RCCSettings.Xbox_startEngineKB))
          KillOrStartEngine();

        if (!string.IsNullOrEmpty(RCCSettingsInstance.Xbox_trailerAttachDetach) &&
            RCC_InputManager.GetButtonDown(RCCSettings.Xbox_trailerAttachDetach))
          DetachTrailer();
        ...
        if (!automaticGear) {
          if (!string.IsNullOrEmpty(RCCSettingsInstance.Xbox_shiftGearUp) &&
              RCC_InputManager.GetButtonDown(RCCSettings.Xbox_shiftGearUp))
            GearShiftUp();

          if (!string.IsNullOrEmpty(RCCSettingsInstance.Xbox_shiftGearDown) &&
              RCC_InputManager.GetButtonDown(RCCSettings.Xbox_shiftGearDown))
            GearShiftDown();
        }
      ...
      case RCC_Settings.ControllerType.PS4:
        ...
        if (!string.IsNullOrEmpty(RCCSettingsInstance.PS4_startEngineKB) &&
            RCC_InputManager.GetButtonDown(RCCSettings.PS4_startEngineKB))
          KillOrStartEngine();

        if (!string.IsNullOrEmpty(RCCSettingsInstance.PS4_trailerAttachDetach) &&
            RCC_InputManager.GetButtonDown(RCCSettings.PS4_trailerAttachDetach))
          DetachTrailer();
        ...
        if (!automaticGear) {
          if (!string.IsNullOrEmpty(RCCSettingsInstance.PS4_shiftGearUp) &&
              RCC_InputManager.GetButtonDown(RCCSettings.PS4_shiftGearUp))
            GearShiftUp();

          if (!string.IsNullOrEmpty(RCCSettingsInstance.PS4_shiftGearDown) &&
              RCC_InputManager.GetButtonDown(RCCSettings.PS4_shiftGearDown))
            GearShiftDown();
        }
  ...
  void FixedUpdate() {
    if ((autoGenerateEngineRPMCurve) && oldEngineTorque != maxEngineTorque ||
        oldMaxTorqueAtRPM != maxEngineTorqueAtRPM || minEngineRPM != oldMinEngineRPM || maxEngineRPM != oldMaxEngineRPM)
      CreateEngineCurve();
    ...
    if (RCCSettings.selectedBehaviorType != null && RCCSettings.selectedBehaviorType.applyRelativeTorque) {
      ...
      if (isGrounded)
        rigid.AddRelativeTorque(
          Vector3.up * (((steerInput * throttleInput) * direction)) * Mathf.Lerp(1.5f, .5f, speed / 100f),
  ...
  private void Engine() {
    ...
    engineRPM = Mathf.SmoothDamp(engineRPM,
      (Mathf.Lerp(minEngineRPM, maxEngineRPM + 500f, (clutchInput * throttleInput)) +
       ((Mathf.Abs(wheelRPM / Mathf.Clamp(poweredWheels, 1, Mathf.Infinity)) * finalRatio *
         (gears[currentGear].maxRatio)) * (1f - clutchInput))) * fuelInput, ref velocity, engineInertia);
  ...
  private void Audio() {
    ...
    if (direction == 1)
      brakeSound.volume = Mathf.Lerp(0f, RCCSettings.maxBrakeSoundVolume,
        Mathf.Clamp01((FrontLeftWheelCollider.wheelCollider.brakeTorque +
                       FrontRightWheelCollider.wheelCollider.brakeTorque) / (brakeTorque * 2f)) *
        Mathf.Lerp(0f, 1f, FrontLeftWheelCollider.wheelCollider.rpm / 50f));
    else
  ...
  private void EngineSounds() {
    ...
    float pitchLevel = Mathf.Lerp(minEngineSoundPitch, maxEngineSoundPitch, engineRPM / maxEngineRPM) *
                       (engineRunning ? 1f : 0f);
  ...
  private void Wheels() {
    for (int i = 0; i < allWheelColliders.Length; i++) {
      if (allWheelColliders[i].canPower)
        allWheelColliders[i].ApplyMotorTorque(
          (direction * allWheelColliders[i].powerMultiplier * (1f - clutchInput) * throttleInput * (1f + boostInput) *
           (engineTorqueCurve.Evaluate(engineRPM) * gears[currentGear].maxRatio * finalRatio)) /
      ...
      if (!appliedBrake && handbrakeInput > .5f) {
        ...
        if (allWheelColliders[i].canHandbrake)
          allWheelColliders[i]
            .ApplyBrakeTorque((brakeTorque * handbrakeInput) * allWheelColliders[i].handbrakeMultiplier);
      }
  ...
  private void AntiRollBars() {
    ...
    if (groundedFL)
      travelFL = (-FrontLeftWheelCollider.transform.InverseTransformPoint(FrontLeftWheelCollider.wheelHit.point).y -
                  FrontLeftWheelCollider.wheelCollider.radius) /
                 FrontLeftWheelCollider.wheelCollider.suspensionDistance;

    bool groundedFR = FrontRightWheelCollider.isGrounded;

    if (groundedFR)
      travelFR = (-FrontRightWheelCollider.transform.InverseTransformPoint(FrontRightWheelCollider.wheelHit.point).y -
                  FrontRightWheelCollider.wheelCollider.radius) /
                 FrontRightWheelCollider.wheelCollider.suspensionDistance;

    float antiRollForceFrontHorizontal = (travelFL - travelFR) * antiRollFrontHorizontal;

    if (groundedFL)
      rigid.AddForceAtPosition(FrontLeftWheelCollider.transform.up * -antiRollForceFrontHorizontal,
        FrontLeftWheelCollider.transform.position);
    if (groundedFR)
      rigid.AddForceAtPosition(FrontRightWheelCollider.transform.up * antiRollForceFrontHorizontal,
    ...
    if (groundedRL)
      travelRL = (-RearLeftWheelCollider.transform.InverseTransformPoint(RearLeftWheelCollider.wheelHit.point).y -
                  RearLeftWheelCollider.wheelCollider.radius) / RearLeftWheelCollider.wheelCollider.suspensionDistance;

    bool groundedRR = RearRightWheelCollider.isGrounded;

    if (groundedRR)
      travelRR = (-RearRightWheelCollider.transform.InverseTransformPoint(RearRightWheelCollider.wheelHit.point).y -
                  RearRightWheelCollider.wheelCollider.radius) /
                 RearRightWheelCollider.wheelCollider.suspensionDistance;

    float antiRollForceRearHorizontal = (travelRL - travelRR) * antiRollRearHorizontal;

    if (groundedRL)
      rigid.AddForceAtPosition(RearLeftWheelCollider.transform.up * -antiRollForceRearHorizontal,
        RearLeftWheelCollider.transform.position);
    if (groundedRR)
      rigid.AddForceAtPosition(RearRightWheelCollider.transform.up * antiRollForceRearHorizontal,
    ...
    if (groundedFL)
      rigid.AddForceAtPosition(FrontLeftWheelCollider.transform.up * -antiRollForceFrontVertical,
        FrontLeftWheelCollider.transform.position);
    if (groundedRL)
      rigid.AddForceAtPosition(RearLeftWheelCollider.transform.up * antiRollForceFrontVertical,
        RearLeftWheelCollider.transform.position);

    float antiRollForceRearVertical = (travelFR - travelRR) * antiRollVertical;

    if (groundedFR)
      rigid.AddForceAtPosition(FrontRightWheelCollider.transform.up * -antiRollForceRearVertical,
        FrontRightWheelCollider.transform.position);
    if (groundedRR)
      rigid.AddForceAtPosition(RearRightWheelCollider.transform.up * antiRollForceRearVertical,
        RearRightWheelCollider.transform.position);
  ...
  private void SteerHelper() {
    ...
    velocityDirection.localRotation = Quaternion.Lerp(velocityDirection.localRotation,
      Quaternion.AngleAxis(Mathf.Clamp(velocityAngle / 3f, -45f, 45f), Vector3.up), Time.fixedDeltaTime * 20f);
    ...
    rigid.AddRelativeTorque(
      Vector3.up * ((angle2 * (Mathf.Clamp(transform.InverseTransformDirection(rigid.velocity).z, -10f, 10f) / 1000f)) *
                    steerHelperAngularVelStrength), ForceMode.VelocityChange);
  ...
  private void TractionHelper() {
    ...
    if (angle * FrontLeftWheelCollider.wheelCollider.steerAngle < 0) {
      FrontLeftWheelCollider.tractionHelpedSidewaysStiffness =
        (1f - Mathf.Clamp01(tractionHelperStrength * Mathf.Abs(angularVelo)));
    }
    else {
    ...
    if (angle * FrontRightWheelCollider.wheelCollider.steerAngle < 0) {
      FrontRightWheelCollider.tractionHelpedSidewaysStiffness =
        (1f - Mathf.Clamp01(tractionHelperStrength * Mathf.Abs(angularVelo)));
    }
    else {
  ...
  private void Clutch() {
    ...
    if (currentGear == 0) {
      if (launched >= .25f)
        clutchInput = Mathf.Lerp(clutchInput,
          (Mathf.Lerp(1f,
            (Mathf.Lerp(clutchInertia, 0f,
              ((wheelRPM) / Mathf.Clamp(poweredWheels, 1, Mathf.Infinity)) / gears[0].targetSpeedForNextGear)),
            Mathf.Abs(throttleInput))), Time.fixedDeltaTime * 20f);
      else
  ...
  public IEnumerator ChangeGear(int gear) {
    ...
    if (gearShiftingClips.Length > 0) {
      gearShiftingSound = NewAudioSource(RCCSettings.audioMixer, gameObject, gearSoundPosition,
        "Gear Shifting AudioSource", 1f, 5f, RCCSettings.maxGearShiftingSoundVolume,
        gearShiftingClips[UnityEngine.Random.Range(0, gearShiftingClips.Length)], false, true, true);
  ...
  private void NOS() {
    ...
    if (!NOSSound)
      NOSSound = NewAudioSource(RCCSettings.audioMixer, gameObject, exhaustSoundPosition, "NOS Sound AudioSource", 5f,
        10f, .5f, NOSClip, true, false, false);
  ...
  private void Turbo() {
    ...
    if (!turboSound) {
      turboSound = NewAudioSource(RCCSettings.audioMixer, gameObject, turboSoundPosition, "Turbo Sound AudioSource",
        .1f, .5f, 0, turboClip, true, true, false);
      NewHighPassFilter(turboSound, 10000f, 10);
    }

    turboBoost = Mathf.Lerp(turboBoost,
      Mathf.Clamp(Mathf.Pow(throttleInput, 10) * 30f + Mathf.Pow(engineRPM / maxEngineRPM, 10) * 30f, 0f, 30f),
    ...
    turboSound.pitch = Mathf.Lerp(Mathf.Clamp(turboSound.pitch, 2f, 3f), (turboBoost / 30f) * 2f,
      ...
      if (colRelVel.magnitude * cos >= minimumCollisionForce) {
        ...
        for (int i = 0; i < deformableMeshFilters.Length; i++) {
          if (deformableMeshFilters[i] != null)
            DeformMesh(deformableMeshFilters[i].mesh, originalMeshData[i].meshVerts, collision, cos,
              deformableMeshFilters[i].transform, rot);
        }
        ...
        if (wheelDamage != WheelDamage.Off) {
          for (int i = 0; i < allWheelColliders.Length; i++) {
            ...
            if (point.magnitude <= wheelDamageRadius) {
              allWheelColliders[i].damagedToe +=
                (UnityEngine.Random.Range(-(colRelVel.magnitude * cos), (colRelVel.magnitude * cos))) *
                wheelDamageMultiplier;
              allWheelColliders[i].damagedCamber +=
                (UnityEngine.Random.Range(-(colRelVel.magnitude * cos), (colRelVel.magnitude * cos))) *
                wheelDamageMultiplier;
              allWheelColliders[i].damagedCaster +=
                (UnityEngine.Random.Range(-(colRelVel.magnitude * cos), (colRelVel.magnitude * cos))) *
                wheelDamageMultiplier;
            }
          }
        ...
        if (crashClips.Length > 0) {
          if (collision.contacts[0].thisCollider.gameObject.transform != transform.parent) {
            crashSound = NewAudioSource(RCCSettings.audioMixer, gameObject, "Crash Sound AudioSource", 5, 20,
              RCCSettings.maxCrashSoundVolume, crashClips[UnityEngine.Random.Range(0, crashClips.Length)], false, true,
              true);
  ...
  void OnDrawGizmos() {
    ...
    if (Application.isPlaying) {
      ...
      for (int i = 0; i < allWheelColliders.Length; i++) {
        ...
        Gizmos.matrix = Matrix4x4.TRS(allWheelColliders[i].transform.position, Quaternion.AngleAxis(-90, Vector3.right),
          Vector3.one);
