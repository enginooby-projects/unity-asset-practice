...
public class RCC_Recorder : MonoBehaviour {
  ...
  public class Recorded {
    ...
    public Recorded(PlayerInput[] _inputs, PlayerTransform[] _transforms, PlayerRigidBody[] _rigids,
      string _recordName) {
      inputs = _inputs;
  ...
  public class PlayerInput {
    ...
    public PlayerInput(float _gasInput, float _brakeInput, float _steerInput, float _handbrakeInput, float _clutchInput,
      float _boostInput, float _fuelInput, int _direction, bool _canGoReverse, int _currentGear, bool _changingGear,
      RCC_CarControllerV3.IndicatorsOn _indicatorsOn, bool _lowBeamHeadLightsOn, bool _highBeamHeadLightsOn) {
  ...
  public void SaveRecord() {
    print("Record saved!");
    recorded = new Recorded(Inputs.ToArray(), Transforms.ToArray(), Rigidbodies.ToArray(),
  ...
  void FixedUpdate() {
    ...
    switch (mode) {
      ...
      case Mode.Record:
        ...
        Inputs.Add(new PlayerInput(carController.throttleInput, carController.brakeInput, carController.steerInput,
          carController.handbrakeInput, carController.clutchInput, carController.boostInput, carController.fuelInput,
          carController.direction, carController.canGoReverseNow, carController.currentGear, carController.changingGear,
          carController.indicatorsOn, carController.lowBeamHeadLightsOn, carController.highBeamHeadLightsOn));
