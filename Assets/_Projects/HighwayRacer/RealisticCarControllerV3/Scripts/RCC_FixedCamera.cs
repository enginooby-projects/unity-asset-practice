...
public class RCC_FixedCamera : MonoBehaviour {
  ...
  void LateUpdate() {
    ...
    RCC_SceneManager.Instance.activePlayerCamera.targetFieldOfView =
      Mathf.Lerp(distance > maxDistance / 10f ? maximumFOV : 70f, minimumFOV, (distance * 1.5f) / maxDistance);
  ...
  public void ChangePosition() {
    ...
    if (Physics.Raycast(target.position, Quaternion.AngleAxis(randomizedAngle, target.up) * target.forward, out hit,
          maxDistance) && !hit.transform.IsChildOf(target) && !hit.collider.isTrigger) {
