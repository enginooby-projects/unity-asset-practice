...
public class RCC_Camera : MonoBehaviour {
  ...
  public Camera
    thisCam; // Camera is not attached to this main gameobject. Camera is parented to pivot gameobject. Therefore, we can apply additional position and rotation changes.
  ...
  public Vector3
    topCameraAngle = new Vector3(45f, 45f, 0f); // If so, we will use this Vector3 angle for top camera mode.

  private float distanceOffset = 0f;

  public float
    maximumZDistanceOffset =
      10f; // Distance offset for top camera mode. Related with vehicle speed. If vehicle speed is higher, camera will move to front of the vehicle.
  ...
  public bool
    TPSAutoFocus = true; // Auto focus to player vehicle. Adjusts distance and height depends on vehicle bounds.
  ...
  internal float
    targetFieldOfView =
      60f; // Camera will adapt its field of view to this target field of view. All field of views below this line will feed this value.
  ...
  private RCC_HoodCamera
    hoodCam; // Hood camera. It's a null script. Just used for finding hood camera parented to the player vehicle.

  private RCC_WheelCamera
    wheelCam; // Wheel camera. It's a null script. Just used for finding wheel camera parented to the player vehicle.
...
//		transform.rotation = playerCar.transform.rotation * Quaternion.AngleAxis(10f, Vector3.right);
  ...
  private void TPS2() {
    ...
    positionTarget += playerCar.transform.rotation *
                      ((-Vector3.forward * TPSDistance) + (Vector3.forward * (playerSpeed / 15f)));
  ...
  private void TPS() {
    ...
    if (!useOrbitInTPSCameraMode) {
      yAngle = Mathf.SmoothDampAngle(transform.eulerAngles.y, playerCar.transform.eulerAngles.y + dir, ref yVelocity,
        1f - TPSRotationDamping);
    }
    else {
      if (orbitX != 0)
        yAngle = Mathf.SmoothDampAngle(transform.eulerAngles.y, playerCar.transform.eulerAngles.y + orbitX_Smoothed,
          ref yVelocity, .025f);
      else
        yAngle = Mathf.SmoothDampAngle(transform.eulerAngles.y, playerCar.transform.eulerAngles.y + dir, ref yVelocity,
          1f - TPSRotationDamping);
  ...
  public void Collision(Collision collision) {
    ...
    if (colRelVel.magnitude * cos >= 5f) {
      ...
      collisionRot =
        Quaternion.Euler(new Vector3(-collisionVector.z * 10f, -collisionVector.y * 10f, -collisionVector.x * 10f));
  ...
  private bool Occluding(Vector3 targetFollow) {
    ...
    if (Physics.Linecast(targetFollow, transform.position, out wallHit,
          ~(1 << LayerMask.NameToLayer(RCCSettings.RCCLayer)))) {
      if (!wallHit.collider.isTrigger && !wallHit.transform.IsChildOf(playerCar.transform))
        return true;
    }
  ...
  public IEnumerator AutoFocus(Transform bounds1, Transform bounds2) {
    ...
    while (timer > 0f) {
      timer -= Time.deltaTime;
      TPSDistance = Mathf.Lerp(TPSDistance,
        ((RCC_GetBounds.MaxBoundsExtent(bounds1) * 2.55f) + (RCC_GetBounds.MaxBoundsExtent(bounds2) * 2.75f)),
        Time.deltaTime);
      TPSHeight = Mathf.Lerp(TPSHeight,
        ((RCC_GetBounds.MaxBoundsExtent(bounds1) * .75f) + (RCC_GetBounds.MaxBoundsExtent(bounds2) * .6f)),
        Time.deltaTime);
      yield return null;
    }
  }
  ...
  public IEnumerator AutoFocus(Transform bounds1, Transform bounds2, Transform bounds3) {
    ...
    while (timer > 0f) {
      timer -= Time.deltaTime;
      TPSDistance = Mathf.Lerp(TPSDistance,
        ((RCC_GetBounds.MaxBoundsExtent(bounds1) * 2.75f) + (RCC_GetBounds.MaxBoundsExtent(bounds2) * 2.75f) +
         (RCC_GetBounds.MaxBoundsExtent(bounds3) * 2.75f)), Time.deltaTime);
      TPSHeight = Mathf.Lerp(TPSHeight,
        ((RCC_GetBounds.MaxBoundsExtent(bounds1) * .6f) + (RCC_GetBounds.MaxBoundsExtent(bounds2) * .6f) +
         (RCC_GetBounds.MaxBoundsExtent(bounds3) * .6f)), Time.deltaTime);
      yield return null;
    }
  }
